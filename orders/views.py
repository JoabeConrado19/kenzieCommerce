...
from shopping_carts.models import ShoppingCarts, ShoppingCartItem
from products.models import Product
from users.models import User
from rest_framework.response import Response
from django.http import Http404
from shopping_carts.serializer import ShoppingCartSerializer
...
class OrderView(generics.ListCreateAPIView, generics.DestroyAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return Response({"message": "Usuário não autorizado para acessar os pedidos"}, status=status.HTTP_401_UNAUTHORIZED)
    def create(self, request, *args, **kwargs):
        ...
        for seller in sellers:
            ...
            for item in cart_items:
                ordered_products_data.append({'product': item.product.id})
            ordered_products_serializer = OrderedProductSerializer(data=ordered_products_data, many=True)
            if ordered_products_serializer.is_valid():
                ...
                return Response(ordered_products_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
